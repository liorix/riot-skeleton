...
(function(process) {
  ...
  Ep.isVolatileContextProperty = function(expr) {
    if (n.MemberExpression.check(expr)) {
      ...
      if (n.Identifier.check(expr.object) && n.Identifier.check(expr.property) && expr.object.name === this.contextId.name && hasOwn.call(volatileContextPropertyNames, expr.property.name)) {
        return true;
      }
    }
    return false;
  ...
  Ep.getContextFunction = function(id) {
    var func = b.functionExpression(id || null, [this.contextId], b.blockStatement([this.getDispatchLoop()]), false, false);
  ...
  Ep.explodeStatement = function(path, labelId) {
    ...
    switch (stmt.type) {
      ...
      case "ForInStatement":
        ...
        self.emitAssign(keyIterNextFn, b.callExpression(util.runtimeProperty("keys"), [self.explodeExpression(path.get("right"))]));
        self.mark(head);
      ...
      case "TryStatement":
        ...
        self.leapManager.withEntry(tryEntry, function() {
          ...
          if (finallyLoc) {
            ...
            self.emit(b.callExpression(self.contextProperty("finish"), [finallyEntry.firstLoc]));
          }
        });
        self.mark(after);
        break;
  ...
  Ep.explodeExpression = function(path, ignoreResult) {
    ...
    function explodeViaTempVar(tempVar, childPath, ignoreChildResult) {
      ...
      if (ignoreChildResult) {} else if (tempVar || (hasLeapingChildren && (self.isVolatileContextProperty(result) || meta.hasSideEffects(result)))) {
    ...
    switch (expr.type) {
      ...
      case "CallExpression":
        ...
        if (!n.MemberExpression.check(oldCalleePath.node) && n.MemberExpression.check(newCallee)) {
          newCallee = b.sequenceExpression([b.literal(0), newCallee]);
        }
        return finish(b.callExpression(newCallee, path.get("arguments").map(function(argPath) {
